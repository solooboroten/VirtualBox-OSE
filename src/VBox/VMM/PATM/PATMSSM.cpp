...
typedef struct _PATCHINFOSSM
{
    ...
    R3PTRTYPE(uint8_t *)  pPrivInstrHC;                             /* todo Can't remove due to structure size dependencies in saved states. */
...
static SSMFIELD const g_aPatmPatchRecFields[] =
{
    ...
    SSMFIELD_ENTRY_IGN_HCPTR(       PATMPATCHRECSSM, patch.pPrivInstrHC),
    ...
    SSMFIELD_ENTRY_IGN_HCPTR(       PATMPATCHRECSSM, patch.cacheRec.pPatchLocStartHC), // saved as zero
    SSMFIELD_ENTRY_IGN_HCPTR(       PATMPATCHRECSSM, patch.cacheRec.pPatchLocEndHC),   // ditto
    SSMFIELD_ENTRY_IGN_RCPTR(       PATMPATCHRECSSM, patch.cacheRec.pGuestLoc),        // ditto
    SSMFIELD_ENTRY_IGNORE(          PATMPATCHRECSSM, patch.cacheRec.opsize),           // ditto
...
static void patmR3PatchConvertSSM2Mem(PPATMPATCHREC pPatch, PPATMPATCHRECSSM pPatchSSM)
{
    ...
    pPatch->patch.pPrivInstrHC        = pPatchSSM->patch.pPrivInstrHC;
    ...
    pPatch->patch.cacheRec            = pPatchSSM->patch.cacheRec;
...
static void patmR3PatchConvertMem2SSM(PPATMPATCHRECSSM pPatchSSM, PPATMPATCHREC pPatch)
{
    ...
    pPatchSSM->patch.pPrivInstrHC        = pPatch->patch.pPrivInstrHC;
    ...
    pPatchSSM->patch.cacheRec            = pPatch->patch.cacheRec;

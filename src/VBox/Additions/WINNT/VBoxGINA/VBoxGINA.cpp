                         ...
                         DWORD *pdwDllVersion)
{
    ...
    DWORD dwRet = VBoxGINALoadConfiguration();
    if (ERROR_SUCCESS != dwRet)
        VBoxGINAVerbose(0, "VBoxGINA: Error loading global configuration, error=%ld\n", dwRet);
    ...
    if (!VBoxGINAHandleCurrentSession())
        VBoxGINAVerbose(0, "VBoxGINA: Handling of remote desktop sessions is disabled.\n");
                          ...
                          PVOID pWinlogonFunctions, PVOID *pWlxContext)
{
    ...
    VBoxGINAReportStatus(VBoxGuestStatusCurrent_Init);
                                 ...
                                 PWSTR pszMprLogonScript, PVOID pEnvironment)
{
     ...
     */
    VBoxGINAReportStatus(VBoxGuestStatusCurrent_Terminated);
...
VOID WINAPI WlxShutdown(PVOID pWlxContext, DWORD ShutdownType)
{
     ...
     */
    VBoxGINAReportStatus(VBoxGuestStatusCurrent_Inactive);
